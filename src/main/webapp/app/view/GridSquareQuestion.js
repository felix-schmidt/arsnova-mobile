...
Ext.define('ARSnova.view.GridSquareQuestion', {
	...
	constructor: function(args) {
		...
		this.gsSaveButton = Ext.create('Ext.Button', {
			...
			text: Messages.SAVE,
			handler: !this.viewOnly ? this.saveGsQuestionHandler : function() {},
			scope: this,
			disabled: true
		});
		...
		var gsContainer = {
			...
			items: [this.gsSaveButton, !!!this.questionObj.abstention ? { hidden: true } : {
				flex: 1,
				xtype: 'button',
				cls: 'login-button noMargin',
				text: Messages.ABSTENTION,
				handler: this.gsAbstentionHandler,
				scope: this
			}]
		};
